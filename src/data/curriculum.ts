    ...
    concepts: [
      {
        ...
        body: [
          ...
          "Consistency, the C, is the odd one out. It describes your application's own rules (your invariants), which the other 3 guarantees help you preserve. The database can only enforce the rules you give it. 'Every approved loan has exactly 1 signoff row' is a consistency invariant. The database can enforce it with a constraint if you tell it to, but the database has no idea what your invariants are unless you say so.",
          "Why this matters before Kubernetes or feature stores make any sense: a model registry, a period-signoff table, an audit trail, every one of those is just a database table with specific ACID guarantees leaned on for a specific reason. When module 3 says 'a model registry tied to a data-versioning tool' should let you name the exact model version live for any past prediction, what's actually doing that work is atomicity (the version pointer and the model artifact get written together or not at all) and durability (once written, it doesn't quietly disappear). Explain which guarantee the tool relies on and why.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          ...
          "The single idea underneath all of it is declarative, reconciled state. To keep 3 containers running, you tell etcd 'the desired state is 3 replicas of this container,' and a control loop in the controller manager continuously checks whether reality matches that, starting new ones if a node dies, stopping extras if you scale down. This is why Kubernetes can recover from a node failure with no human intervention: the reconciliation loop that fixes a typo in a config is the exact same loop that fixes a dead machine. 1 mechanism handles these 2 cases.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          "A Pod is 1 or more containers that always get scheduled together, on the same machine, sharing the same network address and storage. Almost always it's just 1 container, and the reason Kubernetes wraps even a single container in this extra concept is to leave room for the rare case where 2 containers genuinely need to live and die together (a main application container plus a small helper that, say, syncs files into a shared volume). You are not meant to create Pods directly in anything resembling production, because a Pod alone cannot recover from a node failure.",
          "A Pod, on its own, is fragile: if the node it's running on dies, that Pod is just gone, nothing brings it back. A ReplicaSet fixes exactly 1 problem: it watches a set of Pods matching a label and continuously ensures a specific number of them are running, creating new ones if the count drops. It solves 'keep N copies running' and nothing else, no rollout strategy, no history, no rollback.",
          ...
          "These 4 objects solve related problems: Pod solves 'run this container.' ReplicaSet solves 'keep N of them running.' Deployment solves 'change what's running, safely, with history.' Service solves 'let other things find them without caring which specific one answers.' Each layer exists because the one below it left exactly 1 problem unsolved. Learn the object names through the problems they solve.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          ...
          "A message broker makes this reliable at scale. The naive approach (a producer writes to a shared datastore, consumers poll it on a timer) works, but polling gets expensive fast, since most polls find nothing new, and the overhead only grows as you poll more often to reduce delay. A message broker inverts this: producers push events to it, it holds them (in memory, or durably on disk depending on configuration), and consumers get notified as events arrive instead of asking repeatedly. Kafka, Pub/Sub, and similar systems provide a durable, ordered, publish/subscribe log that decouples 'something happened' from 'something acted on it.'",
  ...
  },
  {
    ...
    concepts: [
      ...
      },
      {
        ...
        body: [
          "The standard ML lifecycle diagram is a circle: data in, model trained, model validated, model deployed, model monitored, and eventually retrained on fresh data. In a lending case study, a hiring manager typically wants you to explain where that loop breaks.",
          ...
          "Be ready to explain the 2 or 3 places the loop fails in production and what you'd instrument to catch each failure.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          "If 95% of borrowers repay, a model that predicts 'will repay' for everyone hits 95% accuracy while being completely useless. Credit data has this imbalance, which is why precision, recall, and AUC-PR (not ROC-AUC, which is also misleading under heavy imbalance) show up constantly in credit risk work.",
    ...
    concepts: [
      {
        ...
        body: [
          ...
          "Many systems that call themselves 'streaming and batch' only build 2 things, a batch training pipeline and a real-time inference API, with no genuine event-streaming feature computation (no Kafka, no Pub/Sub) in between. A synchronous read at request time can be good enough if the underlying data source is already fresh. Otherwise, behavioral features are quietly running on whatever the last batch snapshot was, not on this morning's transactions. Ask directly, in an interview or a design review, whether 'streaming' means an actual event pipeline or just 'the API responds fast.' Those are different claims.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          "A feature store is a registry of named, versioned feature definitions, computed once, and read by both the offline training job and the online serving path. Its value comes from the constraint it enforces: nobody can write ad hoc feature logic in a notebook that quietly diverges from what production actually computes.",
      ...
      },
      {
        ...
        body: [
          "The requirement here is traceability: given a specific prediction served last Tuesday, you should be able to name the exact model artifact, the exact feature values, and the exact training data snapshot that produced it. Identify the model exactly, with a hash.",
          "A lending product requires this traceability. When a regulator or an internal audit asks why a specific applicant was denied, 'we're not sure which model version was live that day' is not an acceptable answer. A model registry (MLflow, a cloud provider's model registry, or a homegrown equivalent) tied to a data-versioning tool (DVC or a warehouse snapshot ID) is what makes that answer possible instead of a guess.",
        ],
        whyItMatters:
    ...
    concepts: [
      {
        ...
        body: [
          "Training environments carry a lot of weight: experiment tracking libraries, notebook kernels, GPU drivers, many Python packages. None of that belongs in the serving container. The serving image should hold exactly what's needed to load an artifact and answer a request: the model file, an inference wrapper, and a pinned, minimal runtime.",
          "Separating them limits the blast radius of dependency changes; image size is secondary. A training-only dependency with a security patch, or a version bump that changes some numerical behavior, should never be able to break the live scoring endpoint, because the live endpoint never depended on it in the first place.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          "For a recommendation model, a bad canary means someone sees a worse suggestion for an hour. For a credit model, a bad canary means real people get approved for loans they can't repay, or denied loans they could have handled, and you can't undo either outcome by rolling back the deploy. The damage reaches people's finances.",
  ...
  },
  {
    ...
    concepts: [
      ...
      },
      {
        ...
        body: [
          "An internal ML platform should give data scientists a standard path to production. A data scientist should be able to go from a validated model to a production endpoint with a templated deploy config, a standard monitoring dashboard that comes for free, and a feature-store integration that doesn't require them to understand Kubernetes at all. If they need to file a ticket with the platform team for a routine deploy, the platform has failed at its 1 job.",
          "Evaluate this work by measuring how much median time-to-production drops for a new model, before and after the platform changes. Use that metric to explain whether the platform work is succeeding.",
        ],
        whyItMatters:
    ...
    concepts: [
      {
        ...
        body: [
          ...
          "This is the same underlying idea as the progressive-rollout concept from module 3, but the framing matters in an interview: 'champion/challenger' is the term a credit-risk hiring manager will actually use. Use that term in the interview.",
        ],
        whyItMatters:
    ...
    concepts: [
      {
        ...
        body: [
          "When a live model's approval rate or default rate suddenly shifts, start by checking 3 causes: a pipeline bug feeding it malformed or null features, a genuine shift in the applicant population (a new marketing channel bringing in a different demographic), or actual concept drift, where the real relationship between features and repayment has changed (a macroeconomic shift, for instance).",
      ...
      },
      {
        ...
        body: [
          "Lending is regulated. In most jurisdictions with active fair-lending oversight, a credit model needs both accuracy optimization and monitoring for disparate impact across protected characteristics (even proxies for them, like postal code correlating with ethnicity or income), and every denial typically needs an explainable reason a human can hand to the applicant.",
          "2 things need to run alongside every model from the start of production: a fairness dashboard tracking approval and default rates sliced by demographic proxy groups, watched with the same seriousness as accuracy metrics, and an explainability layer (SHAP values are the common choice) that can turn 'the model said no' into 'insufficient repayment history relative to requested amount,' a reason a compliance team can defend.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          "When a page fires for the scoring service, start with this triage question: is this an infra problem (pods crash-looping, a downstream dependency down) or a model-behavior problem (approval rate or score distribution moved)? Infra problems usually have a fast, safe fix: roll back the deploy or fail over. Model-behavior problems are scarier because the fix isn't always obvious, and a rollback might just trade one bad model for a different, differently-bad one.",
          "For a model-behavior incident, if the platform supports it (see module 3's shadow/canary work), flip a feature flag to route traffic to the last known-good model version immediately, buying time to diagnose the root cause without applicants sitting on a broken decision path in the meantime. Write the incident retro the same day, while the timeline's still fresh, and commit to exactly 1 durable fix.",
        ],
        whyItMatters:
    ...
    concepts: [
      ...
      },
      {
        ...
        body: [
          ...
          "A useful format under interview pressure: state the objective in 1 sentence, list 3 to 5 ordered steps, name the biggest risk in the plan, and name how you'd verify success before declaring done. That format keeps the plan manageable under interview pressure.",
        ],
        whyItMatters:
      ...
      },
      {
        ...
        body: [
          "The best technical plan, explained in jargon nobody in the room can follow, reads as weaker than a simpler plan explained clearly. Practice compressing each module in this curriculum into a 2-sentence explanation a business stakeholder could act on. Translate 'we'll implement a feature store to prevent training-serving skew' into 'we'll make sure the model sees the exact same customer data live as it did during training, so it doesn't make decisions based on stale information.'",
          "A senior engineer needs to know which details matter to each audience. A business stakeholder should be able to understand why a deploy takes 10 minutes without sitting through an explanation of etcd.",
        ],
        whyItMatters:
